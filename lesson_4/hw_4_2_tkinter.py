...
def click() -> None:
    name_user_text = str(name.get())  # Вот так мы получаем текст

    if name_user_text.isalpha() == False:
        # 2. Добавляем виджеты, котороый содержит текст
        label = tk.Label(
            hello_window, text=f"Ошибка: Имя должно стостоять только из букв", fg="red"
        )
        label.pack()
    else:
